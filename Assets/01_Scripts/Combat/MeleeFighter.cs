...
namespace _01_Scripts.Combat
{
    ...
    public class MeleeFighter : MonoBehaviour
    {
        ...
        // Maximum range at which the fighter can perform an attack (Will be computed automatically)
        ...
        public bool IsCountable => AttackState == AttackStates.Windup && (!CombatSettings.Instance.OnlyCounterFirstAttackOfCombo || comboCount == 0);
        public bool IsMatchingTarget { get; private set; } = false;
        public bool IsInSyncedAnimation { get; private set; } = false;
        public AttackData CurrSyncedAction { get; private set; }    // To avoid on attack to prevent ReSetting IsInSyncedAnimation set by another attack

        public bool CanTakeHit { get; set; } = true;    // While false, AI won't try to hit player
        public bool IsInvincible { get; set; } = false;  // While true, the AI will hit the player, but it won't have any effect
        ...
        {
            // Taking Blocked Hit is a special case where counter can be performed
        ...
        public bool ChooseAttacks(MeleeFighter target = null, int combo = 0, bool isHeavyAttack = false, bool isCounter = false, bool isSpecialAttack = false)
        {
            ...
            var normalAttacks = possibleAttacks.Where(a => a.AttackType is AttackType.Single or AttackType.Combo).OrderBy(a => a.MinDistance).ToList();
            ...
            bool inCombo = State == FighterState.Attacking && CurrAttacksList.Count > 0 && combo != CurrAttacksList.Count - 1;
            ...
            if (possibleAttacks.Count > 0)
            {
                ...
                if (!inCombo || (target != null && target.IsKnockedDown))
                {
                    ...
                    possibleAttacks = possibleAttacks.Where(a => Mathf.Approximately(a.HealthThreshold, lowestHealthThreshold)).ToList();
        ...
        public bool ChooseCounterAttacks(MeleeFighter target)
        {
            ...
            if (IsBeingAttacked && CurrAttacker.IsCountable && (!CombatSettings.Instance.OnlyCounterWhileBlocking || WillBlockAttack(CurrAttacker.CurrAttack)))
            {
                Target = CurrAttacker;
                ...
                if (currAttack.CanBeCountered && currAttack.CounterAttacks.Count > 0)
                {
                    ...
                    var counterAttack = possibleCounters.Where(a => Mathf.Approximately(a.HealthThresholdForCounter, lowestHealth)).ToList().
                        GetRandom();
        ...
        IEnumerator Attack(MeleeFighter target = null)
        {
            ...
            if (target != null)
            {
                ...
                if (attack.MoveToTarget)
                {
                    ...
                    if (attack.MoveType == TargetMatchType.ScaleRootMotion)
                    {
                        ...
                        var rootMotionDisplacement = Quaternion.LookRotation(destinationDisplacement) * (endFramePos - startFramePos);
                        ...
                        rootMotionScaleFactor = new Vector3(attack.WeightMask.x * destinationDisplacement.x / rootMotionDisplacement.x, attack.WeightMask.y * destinationDisplacement.y / rootMotionDisplacement.y, attack.WeightMask.z * destinationDisplacement.z / rootMotionDisplacement.z);
                    }
                    moveToPos = targetPos;
            ...
            animGraph.CrossFade(attack.Clip, transitionOut: 0.4f, animationSpeed: attack.AnimationSpeed, clipInfo: attack.Clip);
            ...
            while (animGraph.CurrentClipStateInfo.Timer <= attackLength)
            {
                    ...
                    && attack.MoveToTarget && normalizedTime >= attack.MoveStartTime && normalizedTime <= attack.MoveEndTime)
                {
                    ...
                    {
                        var destinationDisplacement = Vector3.Scale(animator.deltaPosition, rootMotionScaleFactor);
        ...
        void HandleBlockingChanged(bool wasPreviouslyBlocking)
        {
            if (isBlocking && !wasPreviouslyBlocking)
            {
                ...
                animGraph.CrossFadeAvatarMaskAnimation(CurrentWeapon.Blocking, mask: CurrentWeapon.BlockMask, transitionInTime: .1f);
        ...
        void HandleColliderSweep(MeleeFighter target)
        {
            ...
            if (activeBoxCollider && target != null)
            {
                ...
                var size = Physics.OverlapBoxNonAlloc(prevColliderPos, halfExtents, checkCollision, orientation, 1 << target.gameObject.layer, QueryTriggerInteraction.Collide);
                ...
                {
                    bool isHit = Physics.BoxCast(prevColliderPos, halfExtents, direction, out var hit, orientation, distance, 1 << target.gameObject.layer, QueryTriggerInteraction.Collide);
                    //GizmosExtend.drawBoxCastBox(prevColliderPos, halfExtents, orientation, direction, distance);
        ...
        private void OnTriggerEnterAction(Collider other)
        {
            if (other.CompareTag("Hitbox") && CanTakeHit && !IsInvincible)
            {
                ...
                if (IsInSyncedAnimation || State == FighterState.GettingUp || (State == FighterState.TakingHit && currReaction is
                    {
                        willBeKnockedDown: true
                    }))
                    return;
                ...
                // Don't take hit if the victim was not the intented target of the attack
            ...
            AnimationClip getUpAnimation = null)
        {
            ...
            {
                animGraph.CrossFade(reaction.animationClipInfo, transitionBack: CurrentHealth > 0, clipInfo: reaction.animationClipInfo);
                yield return null;
        ...
        public IEnumerator PlayDeathAnimation(MeleeFighter attacker, AnimGraphClipInfo clipInfo = null)
        {
            ...
            yield return animGraph.CrossFadeAsync(clipInfo, transitionBack: false, onComplete: OnDeath, clipInfo: clipInfo);
        ...
        void RotateToAttacker(MeleeFighter attacker, float rotationOffset = 0f)
        {
            var destinationDisplacement = attacker.transform.position - transform.position;
            destinationDisplacement.y = 0;
            transform.rotation = Quaternion.LookRotation(destinationDisplacement) * Quaternion.Euler(new Vector3(0f, rotationOffset, 0f));
        }
        ...
        Reaction ChooseHitReaction(Vector3 hitPoint, AttackData currAttack, ReactionsData reactionData, MeleeFighter attacker = null)
        {
            ...
            reactions = withSameDir.Count() > 0 ? withSameDir.ToList() : reactions.Where(c => c.direction == HitDirections.Any).ToList();
            ...
            if (!String.IsNullOrEmpty(currAttack.ReactionTag))
            {
                ...
                else
                {
                    ...
                    var thatContainsTag = reactionsThatContainsTag as ReactionContainer[] ?? reactionsThatContainsTag.ToArray();
        ...
        IEnumerator SwitchingWeaponStateAnimation(AnimationClip animationClip, float transitionOut, bool playSwitchingAnimation = true, AnimatorOverrideController overrideController = null, WeaponData weaponData = null, Action onComplete = null)
        {
            OnStartAction?.Invoke();
