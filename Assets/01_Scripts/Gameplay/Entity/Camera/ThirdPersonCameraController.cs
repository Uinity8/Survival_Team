...
namespace _01_Scripts.Gameplay.Entity.Characters.Player.Humanoid.Camera
{
    ...
    {
        [Header("Cinemachine 카메라 설정")] [Tooltip("Cinemachine Virtual Camera가 따라갈 대상 오브젝트")]
        public GameObject CinemachineCameraTarget;

        [Tooltip("카메라가 위쪽으로 이동할 수 있는 최대 각도")] public float TopClamp = 70.0f;

        [Tooltip("카메라가 아래쪽으로 이동할 수 있는 최대 각도")] public float BottomClamp = -30.0f;

        [Tooltip("카메라의 기본 회전값을 보정하는 추가 각도 (고정된 시점에서 미세 조정 가능)")]
        public float CameraAngleOverride = 0.0f;

        [Tooltip("카메라 회전 속도")] public float DeltaTimeMultiplier = 1f;
        ...
        private void Start()
        {
            ...
            if (InputManager.Instance)
                InputManager.Instance.OnLookInput += ctx => _look = ctx;
        }
