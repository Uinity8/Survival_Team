...
namespace _01_Scripts.Core
{
    ...
    public class AnimGraphClipInfo : ISerializationCallbackReceiver
    {
        ...
        public void OnAfterDeserialize() 
        {
            if (!serialized)
            {
                TransitionInAndOut = new Vector2(0.2f, 0.2f);
                useGravity = true;
    ...
    public class AnimationEventData
    {
        ...
        public void InvokeCustomAnimationEvent(GameObject gameObj)
        {
            ...
            if (methodInfo != null)
            {
                try
                {
                    List<object> objList = new List<object> { gameObj };
                    foreach (var p in ParametersList)
                    {
                        objList.Add(p);
                    }
                    methodInfo.Invoke(instance, objList.ToArray());
                }
                catch (Exception)
                {
    ...
    public static class TypeCache
    {
        ...
        public static Type FindTypeByName(string className)
        {
            ...
            if (CachedTypes.TryGetValue(className, out Type cachedType))
            {
                return cachedType;
            }
            ...
            if (foundType != null)
            {
                CachedTypes[className] = foundType;
            }
            else
            {
